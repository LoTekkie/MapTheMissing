...
if (!function_exists('mapthemissing_url')) {
    /**
     * Return a fully qualified mapthemissing url
     * @param string $relative_path
     * @param bool $api
     * @return string
     */
    function mapthemissing_url($relative_path='', $api=false) {
        $url = $api ? constant("MAPTHEMISSING_URL_API") : constant('MAPTHEMISSING_URL');
        $url = $url . '/' . ltrim($relative_path, '\/');
        if (str_contains($url, '#')) {
          $url = str_replace('#', constant('MAPTHEMISSING_ATTRIBUTION') . '#', $url);
        } else {
          $url = $url . constant('MAPTHEMISSING_ATTRIBUTION');
        }
        return $url;
    }
}
